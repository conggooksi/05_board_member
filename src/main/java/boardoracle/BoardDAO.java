...
public class BoardDAO {
	...
	public void insertBoard(BoardDTO dto){
		...
		try{
			...
			if(rs.next()){//글이 존재하면
				number=rs.getInt(1)+1;// 최대글번호+1 => 글 그룹으로 사용 하려고 
			}else{//첫번째 글쓰기 일때
				number=1;//ref=number
			}//else-end
			...
			//★★★★★★★
			sql="insert into board(num,writer,subject,pw,"+
			"regdate,ref,re_step,re_level,content,ip) "+
	...
	public List getList(int start,int cnt){
		...
		try{
			...
			pstmt.setInt(1, start);//시작 위치 ★★★★★★★
			pstmt.setInt(2, start+cnt);//어디 까지 
